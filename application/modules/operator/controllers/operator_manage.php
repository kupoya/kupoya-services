...
class Operator_Manage extends Authenticated_Controller {
    ...
    public function create()
    {
        ...
        $this->form_validation->set_rules('operator_name', 'Operator Name', 'required|max_length[45]');
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            log_message('debug', '==> 2');
            $data['data'] = 'error';
            $this->form_validation->set_message('operator', 'Error Message');
            $this->template->build('operator_create', $data);
        }
        else 
        {
            log_message('debug', '==> 3');
            ...
            //$this->form_validation->set_rules('operator_name', 'Operator Name', 'trim|xss_clean');
            $this->load->model('operator/operator_model');
...
//            $data['contact_id'] = '1';
            ...
            $ret = $this->operator_model->save_operator($data);
            $data['data'] = $ret;
