<?php
class Customer_Manage extends Authenticated_Controller {
    ...
    public function index()
    {
        ...
        $data['data'] = $ret;
        $this->template->build('customer_list', $data);
    }
    ...
    public function create()
    {
        ...
        $this->form_validation->set_rules('customer_name', 'Customer Name', 'required|max_length[45]');
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            log_message('debug', '==> 2');
            $data['data'] = 'error';
            $this->form_validation->set_message('customer', 'Error Message');
            $this->template->build('customer_create', $data);
        }
        else 
        {
            log_message('debug', '==> 3');
